...
const RoutinesShow = () => {
  ...
  const uniqueCategories = Array.from(
    new Set(
      coursesData?.map((course) => course.sub_category_id?.category_id.title)
    )
  );
    ...
    (course) =>
      (!selectedSubcategory ||
        (course.sub_category_id &&
          course.sub_category_id.title === selectedSubcategory)) &&
      (!selectedCategory ||
        (course.sub_category_id &&
          course.sub_category_id.category_id.title === selectedCategory))
  );
  ...
  return (
    <div>
      {isLoading && (
        <div className="text-center text-gray-500 mt-4">Routine Loading...</div>
      )}
      ...
      {!isLoading && (
        <>
          {/* Add category filter */}
          <div className="m-8">
            <label className="text-base font-bold">View By Category :</label>
            <select
          ...
          <div className="m-8">
            <label className="text-sm font-bold">View By SubCategory:</label>
            <select
              className="ml-2 p-2 border border-gray-300 rounded"
