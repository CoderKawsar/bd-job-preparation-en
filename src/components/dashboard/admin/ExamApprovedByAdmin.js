...
const ExamApprovedByAdmin = ({ item, i }) => {
  ...
  const { data: questions, isLoading: isFilteredQuestionLoading } =
    useGetAllQuestionsQuery({ exam_id: examId });
  ...
  const handleDelete = async (id) => {
    try {
      const result = await Swal.fire({
        ...
        text: "If you are sure to delete this exam, click 'Yes' otherwise click 'No'.",
        icon: "warning",
    ...
    <tr key={item?._id} className="block md:table-row">
      ...
      <td className="py-2 px-4 border-b text-center md:table-cell">
        <button
          ...
          className={`bg-${
            activeStatus ? "green" : "red"
          }-500 text-white py-1 px-2 rounded-md`}
        >
